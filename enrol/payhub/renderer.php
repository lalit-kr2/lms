 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_payhub_renderer extends plugin_renderer_base {
    ...
    public function manager_page_coupon_manager() {
        ...
        switch ($action) {
            case 'delete':
                require_capability('enrol/payhub:deletecoupon', $this->page->context);
                // We'll do this using a form for now.
                $id = required_param('id', PARAM_INT);
                $this->page->set_title(get_string('coupon:delete', 'enrol_payhub'));
                $coupon = $DB->get_record('enrol_payhub_coupon', array('id' => $id));
                $deleteform = new \enrol_payhub\forms\coupondelete($pageurl, $coupon);
            ...
            case 'edit':
                require_capability('enrol/payhub:editcoupon', $this->page->context);
                $this->page->set_title(get_string('title:couponedit', 'enrol_payhub'));
                $id = required_param('id', PARAM_INT);
                if ($id > 0) {
                    $instance = $DB->get_record('enrol_payhub_coupon', array('id' => $id), '*', MUST_EXIST);
                } else {
                ...
                $form = new \enrol_payhub\forms\coupon($pageurl, $options);
                $pageurl->param('action', 'list');
            ...
            default:
                $strnewform = '';
                if (has_capability('enrol/payhub:createcoupon', $this->page->context)) {
                    $options = new stdClass();
                    $options->instance = (object) array('id' => 0);
                    if ($this->page->course->id <> SITEID) {
                        $options->lockcourse = $this->page->course->id;
                    }
                    $newform = new \enrol_payhub\forms\coupon($pageurl, $options);
                    $newform->process_post($options->instance, $pageurl);
                    $strnewform = '<div class="enrol-payhub-container">' . $newform->render() . '</div>';
                }

                $filter = optional_param('list', 'all', PARAM_ALPHA);
                $table = new \enrol_payhub\tables\coupons($cid, $filter);
                $table->baseurl = $pageurl;
                echo $this->header();
                echo '<div class="enrol-payhub-container">';
                $table->render(25);
                echo '</div>';
                echo $strnewform;
                echo $this->footer();
    ...
    public function admin_page_coupon_manager() {
        ...
        switch ($action) {
            case 'delete':
                require_capability('enrol/payhub:deletecoupon', $this->page->context);
                $this->page->set_title(get_string('title:couponmanager:delete', 'enrol_payhub'));
                // We'll do this using a form for now.
                $id = required_param('id', PARAM_INT);
                $coupon = $DB->get_record('enrol_payhub_coupon', array('id' => $id));
                $deleteform = new \enrol_payhub\forms\coupondelete($pageurl, $coupon);
            ...
            case 'edit':
                require_capability('enrol/payhub:editcoupon', $this->page->context);
                $this->page->set_title(get_string('title:couponmanager:edit', 'enrol_payhub'));
                $id = required_param('id', PARAM_INT);
                if ($id > 0) {
                    $instance = $DB->get_record('enrol_payhub_coupon', array('id' => $id), '*', MUST_EXIST);
                } else {
            ...
            case 'details':
                require_capability('enrol/payhub:config', $this->page->context);
                $this->page->set_title(get_string('title:couponmanager:details', 'enrol_payhub'));
                $id = required_param('id', PARAM_INT);
                $type = optional_param('type', 'all', PARAM_ALPHA);
                $params = array('page' => 'cpcoupons', 'sesskey' => sesskey(), 'type' => $type);
                $back = new \moodle_url('/enrol/payhub/admin.php', $params);
                $table = new \enrol_payhub\tables\couponusage($id);
                ...
                $out = '';
                $out .= $this->header();
                $out .= html_writer::start_div('enrol-payhub-container');
                $out .= html_writer::start_div('enrol-payhub-tabs');
                $out .= $this->admin_tabs('cpcoupons');
                $out .= html_writer::end_div();
                $out .= '<div><a href="' . $back . '">' . get_string('coupons:backtooverview', 'enrol_payhub') . '</a></div>';
                ob_start();
                $table->render(25);
                $out .= ob_get_clean();
                $out .= html_writer::end_div();
            ...
            default:
                $strnewform = '';
                $this->page->set_title(get_string('title:couponmanager', 'enrol_payhub'));
                if (has_capability('enrol/payhub:createcoupon', $this->page->context)) {
                    $options = new stdClass();
                    $options->instance = (object) array('id' => 0);
                    if ($this->page->course->id <> SITEID) {
                    ...
                    $newform = new \enrol_payhub\forms\coupon($pageurl, $options);
                    $newform->process_post($options->instance, $pageurl);
                    $strnewform = html_writer::div($newform->render(), 'enrol-payhub-container');
                }
    ...
    public function admin_page_transactions() {
        global $DB;
        $this->page->set_title(get_string('title:transactions', 'enrol_payhub'));
        $action = optional_param('action', 'list', PARAM_ALPHAEXT);
        $cid = optional_param('cid', 0, PARAM_INT);
        $pageurl = clone $this->page->url;
        $pageurl->param('action', $action);
        ...
        switch ($action) {
            case 'invoice':
                ...
                $transaction = $DB->get_record('enrol_payhub', array('id' => $id));
                $api = new \enrol_payhub\payhub\api();
                $api->request_invoice($transaction, null, null, null, true);
                $pageurl->param('action', null);
                redirect($pageurl, get_string('invoice:requested', 'enrol_payhub'));
                break;
            case 'list':
            default:
                $filter = optional_param('list', 'all', PARAM_ALPHA);
                $table = new \enrol_payhub\tables\payhub($cid, $filter);
                $table->baseurl = $pageurl;
    ...
    public function admin_page_subscription_manager() {
        $this->page->set_title(get_string('title:enrolments', 'enrol_payhub'));
        $action = optional_param('action', 'list', PARAM_ALPHAEXT);
        $cid = optional_param('cid', 0, PARAM_INT);
        $pageurl = clone $this->page->url;
        $pageurl->param('action', $action);
    ...
    public function admin_page_service_manager() {
        $this->page->set_title(get_string('title:service', 'enrol_payhub'));
        $config = get_config('enrol_payhub');
        if (!empty($config->paynlapitoken)) {
            // First, let the forms do their work, if any.
            $form = new \enrol_payhub\payhub\forms\cppapply($this->page->url);
            $form->process_post($this->page->url);

            $cppoform = new \enrol_payhub\payhub\forms\cppoapply($this->page->url);
        ...
        try {
            if (!isset($config->paynlapitoken) || empty($config->paynlapitoken)) {
                $url = new \moodle_url('/enrol/payhub/spapply.php');
                $out .= get_string('api:notconfigured', 'enrol_payhub', $url->out());
            } else {
                $out .= get_string('warn:servicepage', 'enrol_payhub');
                // Display Forms.
                $out .= $form->render();
                $out .= $cppoform->render();
            }
        } catch (\enrol_payhub\payhub\exception $pex) {
            $out .= get_string('paynlconn:remote:error', 'enrol_payhub', $pex->getMessage());
        }
        $out .= html_writer::end_div();
        $out .= $this->footer();
        return $out;
    ...
    public function admin_page_legal() {
        $this->page->set_title(get_string('title:legal', 'enrol_payhub'));
        $out = '';
        $out .= $this->header();
        $out .= html_writer::start_div('enrol-payhub-container');
        $out .= html_writer::start_div('enrol-payhub-tabs');
        ...
        $out .= html_writer::end_div();
        $out .= get_string('admin:page:legal', 'enrol_payhub');
        $out .= html_writer::end_div();
        $out .= $this->footer();
        return $out;
    }
    ...
    protected function admin_tabs($selected, $params = array()) {
        $tabs = array();
        $tabs[] = $this->create_pictab('cpcoupons', 'coupons', 'enrol_payhub',
                new \moodle_url('/enrol/payhub/admin.php', array_merge($params, array('page' => 'cpcoupons'))),
                get_string('cp:coupons', 'enrol_payhub'));
        $tabs[] = $this->create_pictab('cptransactions', 'transactions', 'enrol_payhub',
                new \moodle_url('/enrol/payhub/admin.php', array_merge($params, array('page' => 'cptransactions'))),
                get_string('cp:transactions', 'enrol_payhub'));
        $tabs[] = $this->create_pictab('cpsubscriptions', 'subscriptions', 'enrol_payhub',
                new \moodle_url('/enrol/payhub/admin.php', array_merge($params, array('page' => 'cpsubscriptions'))),
                get_string('cp:subscriptions', 'enrol_payhub'));
        $tabs[] = $this->create_pictab('cpservice', 'paynlconnection', 'enrol_payhub',
                new \moodle_url('/enrol/payhub/admin.php', array_merge($params, array('page' => 'cpservice'))),
                get_string('cp:paynlconnection', 'enrol_payhub'));
        $tabs[] = $this->create_pictab('cplegal', 'legal', 'enrol_payhub',
                new \moodle_url('/enrol/payhub/admin.php', array_merge($params, array('page' => 'cplegal'))),
                get_string('cp:legal', 'enrol_payhub'));
        $tabs[] = $this->create_pictab('cpsettings', 'settings', 'enrol_payhub',
                new \moodle_url('/admin/settings.php', array('section' => 'enrolsettingspayhub')),
                get_string('pluginname', 'enrol_payhub'));
    ...
    public function payment_page_cancel() {
        ...
        $out = '';
        $out .= $this->header();
        $out .= $this->notification(get_string('payment:cancelled', 'enrol_payhub', $this->page->course));
     ...
     * @param stdClass $transactionrecord record from enrol_payhub table
     *
     * @return string full page
     */
    public function payment_page_enrol_status($enrolled, $course, $transactionrecord) {
        ...
        if ($enrolled) {
            ...
            $out .= $this->box('<p style="text-align: center">' . get_string('enrol:ok', 'enrol_payhub', $course) . '</p>');
            ...
            $out .= $this->box('<p style="text-align: center">' . get_string('enrol:fail', 'enrol_payhub', $course) .
                    '</p><p style="text-align: center">' .
                    get_string('enrol:fail:tx', 'enrol_payhub', $transactionrecord) . '</p>');
            $out .= $this->continue_button($CFG->wwwroot . '/my');
        }
        $out .= $this->footer();
        return $out;
